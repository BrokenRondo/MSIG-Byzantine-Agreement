...
import (
	...
	mbytes "github.com/ethereum/go-ethereum/consensus/multisig/types"
...
type ProtocolManager struct {
	...
	// MSIG parameters
	multisigdb              ethdb.Database // bft database
...
func NewProtocolManager(config *params.ChainConfig, networkId uint64, mux *event.TypeMux, txpool *core.TxPool, blockchain *core.BlockChain, chaindb ethdb.Database, multisigdb ethdb.Database, vmConfig vm.Config, validators []common.Address, privateKeyHex string, etherbase common.Address, allowEmpty bool, byzantineMode int) (*ProtocolManager, error) {
	...
	manager.consensusManager = NewConsensusManager(manager, blockchain, multisigdb, manager.consensusContract, manager.privateKeyHex)
...
func (pm *ProtocolManager) handleBFTMsg(p *peer) error {
	...
	case msg.Code == GetPrecommitLocksetsMsg:
		...
		var found []*mbytes.PrecommitLockSet
	...
	case msg.Code == PrecommitLocksetMsg:
		var pls []*mbytes.PrecommitLockSet
...
func (pm *ProtocolManager) BroadcastBFTMsg(msg interface{}) {
	...
	case *mbytes.BlockProposal:
		peers := pm.peers.PeersWithoutHash(m.Hash())
		// log.Info("Send Bp: ", m)
		for _, peer := range peers {
			peer.SendNewBlockProposal(m)
		}
	case *mbytes.VotingInstruction:
		peers := pm.peers.PeersWithoutHash(m.Hash())
...
func (pm *ProtocolManager) BroadcastTwoBlockProposal(bp1 *mbytes.BlockProposal, bp2 *mbytes.BlockProposal) {
	var err error
