...
import (
	...
	mtypes "github.com/ethereum/go-ethereum/consensus/multisig/types"
...
// Editing to multisig protocal:
// Each proposal of a round will have f+1 leaders' signature
// So that we do need (f+1) leaders in each round
...
func (cc *ConsensusContract) isProposer(p mtypes.Proposal) bool {
...
type ConsensusManager struct {
	...
	readyNonce              uint64
	blockCandidates         map[common.Hash]*mtypes.BlockProposal
	hdcDb                   ethdb.Database
	synchronizer            *Synchronizer
	// lastCommittingLockset   *mtypes.LockSet
...
func NewConsensusManager(manager *ProtocolManager, chain *core.BlockChain, db ethdb.Database, cc *ConsensusContract, privkeyhex string) *ConsensusManager {
	...
	cm := &ConsensusManager{
		...
		readyNonce:         0,
		blockCandidates:    make(map[common.Hash]*mtypes.BlockProposal),
		contract:           cc,
...
func (cm *ConsensusManager) initializeLocksets() {
	...
	v := mtypes.NewPrecommitVote(0, 0, cm.chain.Genesis().Hash(), 1) // voteBlock
...
func (cm *ConsensusManager) storeLastCommittingLockset(ls *mtypes.PrecommitLockSet) error {
...
func (cm *ConsensusManager) loadLastCommittingLockset() *mtypes.PrecommitLockSet {
	key := fmt.Sprintf("last_committing_lockset")
	data, _ := cm.hdcDb.Get([]byte(key))
	if len(data) == 0 {
		return nil
	}
	var lockset *mtypes.PrecommitLockSet
...
func (cm *ConsensusManager) storePrecommitLockset(blockhash common.Hash, pls *mtypes.PrecommitLockSet) error {
...
func (cm *ConsensusManager) loadPrecommitLockset(blockhash common.Hash) *mtypes.PrecommitLockSet {
	key := fmt.Sprintf("precommitLockset:%s", blockhash)
	data, _ := cm.hdcDb.Get([]byte(key))
	if len(data) == 0 {
		return nil
	}
	var pls *mtypes.PrecommitLockSet
...
func (cm *ConsensusManager) getPrecommitLocksetByHeight(height uint64) *mtypes.PrecommitLockSet {
	if height >= cm.Height() {
...
func (cm *ConsensusManager) commitPrecommitLockset(hash common.Hash, pls *mtypes.PrecommitLockSet) {
	cm.writeMapMu.Lock()
...
// Deleting this func for Uni-Quorum
...
func (cm *ConsensusManager) Sign(s interface{}) {
	log.Debug("CM Sign")
	switch t := s.(type) {
	case *mtypes.BlockProposal:
		t.Sign(cm.privkey)

	//Deleting Vote for Uni-Quorum
	case *mtypes.Vote:
		t.Sign(cm.privkey)
	case *mtypes.PrecommitVote:
		t.Sign(cm.privkey)
	case *mtypes.LockSet:
		t.Sign(cm.privkey)
	case *mtypes.PrecommitLockSet:
		t.Sign(cm.privkey)
	case *mtypes.VotingInstruction:
		t.Sign(cm.privkey)
	case *mtypes.Ready:
		t.Sign(cm.privkey)
	default:
...
func (cm *ConsensusManager) SendReady(force bool) {
	...
	r := mtypes.NewReady(cm.readyNonce, ls)
	cm.Sign(r)
	r.From()
	cm.broadcast(r)
	cm.readyNonce += 1
...
func (cm *ConsensusManager) AddReady(ready *mtypes.Ready) {
	cc := cm.contract
	addr, err := ready.From()
	if err != nil {
...
// Deleting this func for Uni-Quorum
func (cm *ConsensusManager) AddVote(v *mtypes.Vote, peer *peer) bool {
	if v == nil {
...
func (cm *ConsensusManager) AddPrecommitVote(v *mtypes.PrecommitVote, peer *peer) bool {
	if v == nil {
...
func (cm *ConsensusManager) AddProposal(p mtypes.Proposal, peer *peer) bool {
	if p == nil {
...
func (cm *ConsensusManager) addBlockProposal(bp *mtypes.BlockProposal) bool {
...
func (cm *ConsensusManager) addBlockCandidates(bp *mtypes.BlockProposal) {
	cm.writeMapMu.Lock()
	cm.blockCandidates[bp.Blockhash()] = bp
	cm.writeMapMu.Unlock()
}

func (cm *ConsensusManager) lastCommittingLockset() *mtypes.PrecommitLockSet {
...
func (cm *ConsensusManager) HighestCommittingLockset() *mtypes.PrecommitLockSet {
	var hcls *mtypes.PrecommitLockSet
	hcls = nil
...
func (cm *ConsensusManager) lastValidLockset() *mtypes.LockSet {
...
func (cm *ConsensusManager) lastValidPrecommitLockset() *mtypes.PrecommitLockSet {
	// log.Debug("cm lastValidPrecommitLockset ")
	ls := cm.getHeightManager(cm.Height()).lastValidPrecommitLockset()
	return ls
}

func (cm *ConsensusManager) lastLock() *mtypes.Vote {
	return cm.getHeightManager(cm.Height()).LastVoteLock()
}

func (cm *ConsensusManager) mkLockSet(height uint64) *mtypes.LockSet {
	return mtypes.NewLockSet(cm.contract.numEligibleVotes(height), []*mtypes.Vote{})
}

func (cm *ConsensusManager) mkPLockSet(height uint64) *mtypes.PrecommitLockSet {
	return mtypes.NewPrecommitLockSet(cm.contract.numEligibleVotes(height), []*mtypes.PrecommitVote{})
}
...
//Deleting this func for Uni-Quorum
func (hm *HeightManager) LastVoteLock() *mtypes.Vote {
	// highest lock
...
func (hm *HeightManager) LastPrecommitVoteLock() *mtypes.PrecommitVote {
	// highest lock
	for i := len(hm.rounds) - 1; i >= 0; i-- {
		index := uint64(i)
		if hm.getRoundManager(index).voteLock != nil {
...
func (hm *HeightManager) LastVotedBlockProposal() *mtypes.BlockProposal {
...
func (hm *HeightManager) lastValidLockset() *mtypes.LockSet {
...
func (hm *HeightManager) lastValidPrecommitLockset() *mtypes.PrecommitLockSet {
	// highest valid lockset on height
	for i := len(hm.rounds) - 1; i >= 0; i-- {
		index := uint64(i)
...
// PoLC_Lockset
func (hm *HeightManager) lastQuorumLockset() *mtypes.LockSet {
	var found *mtypes.LockSet
...
func (hm *HeightManager) lastQuorumPrecommitLockSet() *mtypes.PrecommitLockSet {
	var found *mtypes.PrecommitLockSet
	for i := 0; i < len(hm.rounds); i++ {
		index := uint64(i)
...
func (hm *HeightManager) addVote(v *mtypes.Vote, process bool) bool {
	addr, _ := v.From()
...
func (hm *HeightManager) addPrecommitVote(v *mtypes.PrecommitVote, process bool) bool {
	addr, _ := v.From()
	if !hm.cm.contract.isValidators(addr) {
		log.Debug("non-validator vote")
		return false
...
func (hm *HeightManager) addProposal(p mtypes.Proposal) bool {
...
type RoundManager struct {
	...
	round             uint64
	height            uint64
	lockset           *mtypes.LockSet
	precommitLockset  *mtypes.PrecommitLockSet
	proposal          mtypes.Proposal
	voteLock          *mtypes.Vote
	precommitVoteLock *mtypes.PrecommitVote
	timeoutTime       float64
...
func (rm *RoundManager) addVote(vote *mtypes.Vote, force_replace bool, process bool) bool {
...
func (rm *RoundManager) addPrecommitVote(vote *mtypes.PrecommitVote, force_replace bool, process bool) bool {
...
func (rm *RoundManager) addProposal(p mtypes.Proposal) bool {
...
func (rm *RoundManager) process() {
	...
	if rm.cm.Config.AlwaysAgree {
		if rm.voteLock == nil && rm.proposal != nil {
			...
			vote := mtypes.NewVote(rm.height, rm.round, blockhash, 1)
			precommitVote := mtypes.NewPrecommitVote(rm.height, rm.round, blockhash, 1)
...
func (rm *RoundManager) propose() mtypes.Proposal {
	...
	if rm.cm.Config.DifferentProposal == true {
		...
		if bp := rm.mkProposal(); bp != nil {
			...
			var roundLockset *mtypes.LockSet
			if bp.Round == 0 {
				roundLockset = nil
			} else {
			...
			if bp2, err := mtypes.NewBlockProposal(bp.Height, bp.Round, block, bp.SigningLockset, roundLockset); err == nil && bp2 != nil {
	...
	if roundLockset == nil && rm.round == 0 {
		...
		if !quorum {
			...
			if p, err := mtypes.NewVotingInstruction(rm.height, rm.round, roundLockset); err != nil {
				log.Error("error occur %v", err)
				return nil
			} else {
				proposal = p
...
func (rm *RoundManager) mkProposal() *mtypes.BlockProposal {
	var roundLockset *mtypes.LockSet
	...
	blockProposal, err := mtypes.NewBlockProposal(rm.height, rm.round, block, signingLockset, roundLockset)
	if err != nil {
		log.Error("error occur %v", err)
		return nil
	}
...
func (rm *RoundManager) vote() *mtypes.Vote {
	...
	if lastPrecommitVoteLock != nil && lastPrecommitVoteLock.VoteType == 1 {
		...
		case *mtypes.VotingInstruction: // vote for votinginstruction
			...
			if quorum && bp.LockSet().Round() > lastPrecommitVoteLock.Round {
				...
				vote = mtypes.NewVote(rm.height, rm.round, bp.Blockhash(), 1)
			} else {
				if lastPrecommitVoteLock == nil {
					vote = mtypes.NewVote(rm.height, rm.round, common.StringToHash(""), 2)
				}
			}
		case *mtypes.BlockProposal:
			log.Debug("voting on new proporsal")
			vote = mtypes.NewVote(rm.height, rm.round, rm.proposal.Blockhash(), 1)
		}
...
func (rm *RoundManager) votePrecommit() *mtypes.PrecommitVote {
	if rm.precommitVoteLock != nil {
		log.Debug("precommit voted")
		return nil
	}
	var vote *mtypes.PrecommitVote
	if rm.lockset.IsValid() {
		if quorum, blockhash := rm.lockset.HasQuorum(); quorum {
			log.Debug("prevote quorum. vote precommit on block")
			vote = mtypes.NewPrecommitVote(rm.height, rm.round, blockhash, 1)
		} else if rm.timeoutTime != 0 && float64(rm.cm.Now()) >= rm.timeoutTime {
			log.Debug("prevote no quorum. vote precommit nil")
			vote = mtypes.NewPrecommitVote(rm.height, rm.round, common.StringToHash(""), 2)
		} else {
